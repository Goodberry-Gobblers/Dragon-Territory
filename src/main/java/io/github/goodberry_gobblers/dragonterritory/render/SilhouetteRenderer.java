...
//        RenderSystem.depthMask(true);
    ...
    public static void renderAt(BufferBuilder bufferbuilder, Matrix4f matrix4f, int size, float angleOffset, float radius, float height, float blocksPerSecond, boolean clockwise, Vector3f color, int renderTick, float partialTick) {
        ...
        float uvy1 = (frameId+1)/3f;
        float uvy2 = frameId/3f;
    ...
    private static List<Silhouette> generateSilhouettes(RandomSource randomSource) {
        ...
        for (int i = 0; i < maxDragonCount; ++i) {
            list.add(new Silhouette(
                    ...
                    randomSource.nextIntBetweenInclusive(20, 150),
    ...
    private static class Silhouette {
        ...
        public Silhouette(int size, float offsetAngle, float radius, float height, float blocksPerSecond, boolean clockwise) {
            this.size = size;
